...
public:
    /**
     * Construct a RendererVlk instance using the given 'game' as an event-emitter.
     */
    explicit RendererVk(kw::IGame* game) noexcept;

    RendererVk(const RendererVk& original) = delete;

    RendererVk& operator=(const RendererVk& original) = delete;
...
private:
    ...
    VkPresentModeKHR choose_swap_present_mode(const kw::Vector<VkPresentModeKHR>& available_present_modes);
