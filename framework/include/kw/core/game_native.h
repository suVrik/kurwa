...
namespace kw {
/**
 * Game native is a class that encapsulates SDL calls.
 ...
 *
 * Listeners of GameNative's signals 'on_event' and 'on_update' are allowed to throw exceptions
 * (prefer std::runtime_error or its child, because then GameNative can produce a meaningful error message).
 */
class GameNative {
    public:
        /**
        ...
        Signal<void(SDL_Event&)> on_event; /// Is emitted for every single event before an update signal.
